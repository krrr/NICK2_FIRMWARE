...
SPICustomClass::SPICustomClass() {
    useHwCs = false;
    pinSet = SPI_PINS_HSPI;
}

bool SPICustomClass::pins(int8_t sck, int8_t miso, int8_t mosi, int8_t ss)
{
    if (sck == 6 &&
        miso == 7 &&
        mosi == 8 &&
...
void SPICustomClass::setHwCs(bool use) {
    switch (pinSet) {
    case SPI_PINS_HSPI:
        if (use) {
...
void SPICustomClass::beginTransaction(SPICustomSettings settings) {
...
void SPICustomClass::endTransaction() {
}

void SPICustomClass::setDataMode(uint8_t dataMode) {
...
void SPICustomClass::setBitOrder(uint8_t bitOrder) {
...
void SPICustomClass::setFrequency(uint32_t freq) {
...
void SPICustomClass::setClockDivider(uint32_t clockDiv) {
...
void IRAM_ATTR SPICustomClass::setDataBits(uint16_t bits) {
    const uint32_t mask = ~((SPIMMOSI << SPILMOSI) | (SPIMMISO << SPILMISO));
    bits--;
    SPI1U1 = ((SPI1U1 & mask) | ((bits << SPILMOSI) | (bits << SPILMISO)));
}

uint8_t IRAM_ATTR SPICustomClass::transfer(uint8_t data) {
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
uint8_t IRAM_ATTR SPICustomClass::transferFast(uint8_t data) {
    //while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
uint16_t SPICustomClass::transfer16(uint16_t data) {
    union {
            uint16_t val;
            struct {
                    uint8_t lsb;
...
void SPICustomClass::transfer(void *buf, uint16_t count) {
...
void SPICustomClass::write(uint8_t data) {
    while(SPI1CMD & SPIBUSY) {}
    // reset to 8Bit mode
    setDataBits(8);
    SPI1W0 = data;
...
void SPICustomClass::write16(uint16_t data) {
    write16(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void SPICustomClass::write16(uint16_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
...
void SPICustomClass::write32(uint32_t data) {
    write32(data, !(SPI1C & (SPICWBO | SPICRBO)));
}

void SPICustomClass::write32(uint32_t data, bool msb) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 * @param size uint32_t
 */
void SPICustomClass::writeBytes(const uint8_t * data, uint32_t size) {
    while(size) {
        if(size > 64) {
            writeBytes_(data, 64);
            size -= 64;
...
void SPICustomClass::writeBytes_(const uint8_t * data, uint8_t size) {
    while(SPI1CMD & SPIBUSY) {}
 ...
 */
void SPICustomClass::writePattern(const uint8_t * data, uint8_t size, uint32_t repeat) {
 ...
 * @param size uint32_t
 */
void SPICustomClass::transferBytes(const uint8_t * out, uint8_t * in, uint32_t size) {
    while(size) {
        if(size > 64) {
...
void SPICustomClass::transferBytesAligned_(const uint8_t * out, uint8_t * in, uint8_t size) {
    if (!size)
        return;
...
void SPICustomClass::transferBytes_(const uint8_t * out, uint8_t * in, uint8_t size) {
