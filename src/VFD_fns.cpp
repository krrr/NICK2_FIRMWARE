  ...
  */
  ...
  if (!isPoweredOn) {
    for (int i = 0; i < DIGITS_COUNT; i++) {
      for (int ii = 0; ii < CATHODE_COUNT; ii++) {
...
void initScreen() {
  ...
  SPICustom.begin();
  SPICustom.setDataMode(SPI_CUSTOM_MODE0);
  SPICustom.setBitOrder(MSBFIRST);
  SPICustom.setClockDivider(SPI_CLOCK_DIV16);
...
void cycleDigits() {
  ...
  for (int i = 0; i < 10; i++) {
    for (int ii = 0; ii < DIGITS_COUNT; ii++) {
      setDigit(ii, i);
    }
    delay(1000);
  }
...
void setupPhaseShift() {
  ...
  disableScreen();
  uint8_t shiftSteps = floor(pwmResolution / DIGITS_COUNT);
  if (shiftSteps)
    for (int i = 0; i < DIGITS_COUNT; i++) {
